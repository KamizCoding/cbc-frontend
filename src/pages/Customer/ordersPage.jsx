...
export default function OrdersPage() {
    ...
    return (
        <div className="overflow-y-auto min-h-screen flex flex-col bg-primary">
            <div className="flex-grow w-full flex flex-col items-center px-4">
                <h1 className="text-3xl font-bold text-dark my-6">Your Orders</h1>
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <thead className="sticky top-0 bg-secondary z-10 shadow-md">
                                    <tr className="text-primary">
                                        <th className="px-5 py-4 rounded-tl-lg">Order ID</th>
                                        <th className="px-5 py-4">Status</th>
                                        <th className="px-5 py-4">Date</th>
                                        <th className="px-5 py-4 rounded-tr-lg">Total</th>
                                    </tr>
                                </thead>
                                ...
                                <tbody>
                                    {orders.map((order, index) => (
                                        <tr 
                                            key={order.orderId} 
                                            className={`border-b border-accent cursor-pointer transition-all 
                                                        ${index % 2 === 0 ? "bg-gray-50" : "bg-gray-100"} 
                                                        hover:bg-gray-200`}
                                            onClick={() => openModal(order)}
                                        >
                                            <td className="px-5 py-4 text-sm text-gray-800">{order.orderId}</td>
